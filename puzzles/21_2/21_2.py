...
# print(totalCount)
finalAllergenIngredientMap = {}
for i in range(len(allergensToPossibleIngredients)):
    for allergen, ingredients in allergensToPossibleIngredients.items():
        if len(ingredients) == 1:
            finalAllergenIngredientMap[allergen] = ingredients.pop()
        for ingredient in finalAllergenIngredientMap.values():
            if ingredient in ingredients:
                ingredients.remove(ingredient)

print(finalAllergenIngredientMap)
dangerousIngredients = []
for allergen in sorted(finalAllergenIngredientMap):
    dangerousIngredients.append(finalAllergenIngredientMap[allergen])
